...
test("current release docs route through structured identities", async () => {
  ...
  assert.match(
    releasing,
    /Both pairs passed for the earlier package[\s\S]*?2026-08-30-claude-plugin-0\.2\.1-two-turn-smokes\.md[\s\S]*?root-handoff and GitHub repository guidance change packaged Skill bytes[\s\S]*?does not qualify these new\s+bytes for publication/,
  );
