...
test("bounded import evals bind supported and unsupported Plan state", async () => {
  ...
  assert.match(
    readme,
    /`initialize-empty-plan`, `author-without-local-validator`, `push-supported-enum-plan`, and\s+`repair-well-founded-analysis-issue` are server-backed evals/,
  );
  assert.match(
    readme,
  ...
  assert.match(
    readme,
    /Before every run,\s+replace it with `.firstdraft\/state\.json`\s+generated by a fresh\s+`firstdraft plan init`[\s\S]*?in a scratch\s+directory before staging it/,
  );
...
test("analysis status guidance follows the pinned CLI contract", async () => {
  ...
  const statusReference = recoveryReference.match(
    /## Whole-graph analysis status([\s\S]*?)## Diagnostics response/,
  );
  ...
  assert.match(
    statusReference[1],
    /`valid` is the gate that a future Compilation action will require[\s\S]*?does not yet implement\s+Compilation/,
  );
  assert.match(
    statusReference[1],
  ...
  assert.match(
    readme,
    /The `\*-analysis\.json` fixtures are behavioral examples accepted by the pinned CLI contract[\s\S]*?not evidence\s+that the pending server AnalysisRun slice is merged, deployed, or released/,
  );
  assert.match(
    readme,
    /Until that server slice lands[\s\S]*?cannot\s+be graded as though a terminal `analysis\.status` were reachable/,
  );
...
async function checkSkill(skillName) {
  ...
  assert.equal(
    defaultPrompt,
    `Use $${skillName} to help me author and review an experimental First Draft Foundation Plan. Keep it local unless I explicitly approve sending the complete Plan for bounded import and whole-graph analysis.`,
  );
}
