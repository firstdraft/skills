...
Movie Preview is a film catalog for viewers to browse movies and track watched status on their existing bookmarks.
Bookmark views and updates are limited to the signed-in viewer.
...
The samples include five fictional movies. Demo Viewer has two watched and two unwatched bookmarks, while
Empty Viewer has none.

After setup and starting the app, open `/login` on your private development preview and use an initial demo login:

- Email: <code>viewer@movie-preview.example</code>; initial password: <code>MoviePreview-Demo-Only-2026!</code>.

- Email: <code>empty@movie-preview.example</code>; initial password: <code>MoviePreview-Demo-Only-2026!</code>.

These deliberately public credentials are for disposable local development Accounts only. Never reuse a real
password. Seeding preserves an existing password, so after changing it, sign in with your chosen password instead
of the initial value above. Demo Accounts are created only in development; they are not production credentials.
...
Read [UI.md](UI.md) for component and interaction conventions, [AGENTS.md](AGENTS.md) for agent guidance, and
