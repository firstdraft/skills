...
const sourceRoot = "/Users/sandbox2/code/firstdraft/ui-account-release-20260915/skills-update";
const sourceSha = "54294d6cf4d1a45f5a21c7d1036b9fddab9d911a";
const packageSha = "d9f466076a05e867019bc3e20c35b89076cc83195e781d9203c039d166becde6";
const pluginId = "firstdraft@firstdraft-skills";
const canonical = Object.fromEntries(files(path.join(sourceRoot, "skills/create-full-stack-app")).map(file => [path.relative(sourceRoot,file),createHash("sha256").update(readFileSync(file)).digest("hex")]));
...
if (values.prepare) {
  ...
  jsonFile(path.join(root, "preparation.json"), {
    ...
    expected_source: sourceSha, package_version: "0.2.5", package_sha256: packageSha,
...
for (const client of values.client === "both" ? ["claude", "codex"] : [values.client]) {
  ...
  assert.equal(matching[0].version, "0.2.5");
  ...
  for (const file of manifestFiles) {
    ...
    if (manifest.name !== "firstdraft" || manifest.version !== "0.2.5") continue;
  ...
  const result = { client, client_version: versions[client], catalog_commit: catalogSha, catalog_root: catalogRoot, plugin_id: pluginId, plugin_version: "0.2.5", enabled: true, installed_root: installedRoot, skill_locator: loadedSkill, canonical_files_match: 9, bundled_cli_version: "0.2.2", no_global_cli: true, local_key_generation: true, credentials_supplied: false, model_or_firstdraft_service_invoked: false, public_commands_passed: true };
  jsonFile(path.join(clientRoot, "receipt.json"), result);
  results.push(result);
  console.log(`${client}: fresh public installation passed`);
}
const metadataResponse = await fetch("https://registry.npmjs.org/@firstdraft.com%2fclaude-code/0.2.5", { signal: AbortSignal.timeout(30_000) });
